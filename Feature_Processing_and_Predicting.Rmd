...
```{r}
#this codes are from github primary objects
...
males$label <- 1 
females$label <- 0
males$label <- as.factor(males$label)
females$label <-  as.factor(females$label)
...
voices$label1 <- as.factor(voices$label1)
#remove column label
...
```{r}
pred4 <- predict(rf_model, newdata = data)
confusion_matrix <- table(prediction = pred4, actual = data$label)
addmargins(confusion_matrix)
```

```{r}
predict(rf_model, newdata = data)
```



```{r}
data.frame(name = data$sound.files, predict = pred4, actual = data$label)
```
